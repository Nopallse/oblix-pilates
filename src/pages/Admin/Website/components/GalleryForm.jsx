...
import { useGalleryForm } from '../api'
import { useApiToast } from '@shared/hooks'

const GalleryForm = ({ isOpen, onClose, gallery = null, onSuccess, createGallery, updateGallery, fetchGalleries }) => {
  ...
  const { formik, loading } = useGalleryForm(
    gallery ? {
      title: gallery.title,
  ...
  const [imagePreview, setImagePreview] = useState(gallery?.picture || null)

  // Tambahkan efek ini agar imagePreview update saat gallery berubah
  useEffect(() => {
    // Jika ada gallery picture, gunakan full URL
    if (gallery?.picture) {
  ...
  const handleImageChange = (event) => {
    ...
    if (file) {
      ...
      if (isEdit && gallery?.picture) {
        formik.setFieldValue('picture', gallery.picture)
        setImagePreview(getImageUrl(gallery.picture))
  ...
  const handleFormSubmit = async (values) => {
    try {
      ...
      if (isEdit) {
        // For edit mode, compare with original gallery data
        if (values.title !== gallery.title) {
      ...
      console.log('Original gallery data:', gallery)
